...
const refresh = async () => {
	...
	await service.dict.info.data({ types: ["subOrderType"] }).then((res) => {
		list.value.push(...res["subOrderType"]);
...
function rowClick(item?: any) {
	if (!item) {
		...
		var title = !isEmpty(props.orderInfo) ? `${props.orderInfo?.projectNum} - 全部` : "全部";
		ViewGroup.value?.setTitle(`子订单列表 (${title})`);
		return;
	}

	if (item) {
		// 刷新列表
		emit("refresh", { page: 1, orderType: item.value });
		// 选择
		ViewGroup.value?.select(item);
		let title = !isEmpty(props.orderInfo)
			? `${props.orderInfo?.projectNum} -  ${item.name}`
			: item.name;
		ViewGroup.value?.setTitle(`子订单列表（${title})`);
	}
}
